...
namespace HttpMachine
{
    public class HttpCombinedParser : IHttpCombinedParser, IDisposable
    {
		...
		private readonly IObserver<ParserStatus> _observerParserStatus;
		...
		public readonly IObservable<ParserStatus> ParserStatusObservable;
        ...
        public HttpCombinedParser(IHttpParserCombinedDelegate del) : this()
        {
			...
			var parserStateSubject = new BehaviorSubject<ParserStatus>(ParserStatus.Start);

            _observerParserStatus = parserStateSubject.AsObserver();
            ParserStatusObservable = parserStateSubject.AsObservable();
        }
